...
def _generate_quadratic_chirp(t_axis: np.ndarray, a: float) -> np.ndarray:
    """Generates a quadratic chirp.
    ...
    Returns:
        The samples of a quadratic chirp along the given time axis.
    """
...
def plot_non_uniform_sampling_quadratic() -> None:
    ...
    x = _generate_quadratic_chirp(t, a)
    n = np.arange(LENGTH) / fs
    samples = _generate_quadratic_chirp(n, a)
    ...
    ax[0].set_title("Uniform samples of nonlinear chirp")

    t_non_uniform = np.linspace(0, (LENGTH / fs)**2, 10000)
    x_non_uniform = _generate_quadratic_chirp(np.sqrt(t_non_uniform), a)
