"""Optimizes the HFSS design variables for a patch antenna with an SMA
connector.
"""
...
OPTIMIZER_VARIABLES = [
    OptimizerVariable(
        "w_patch",
        initial_value=11.527758223416729,
        lower_bound=9,
        upper_bound=13,
        unit="mm",
    ),
    OptimizerVariable(
        "l_patch",
        initial_value=7.706365226108972,
        lower_bound=6,
        upper_bound=10,
        unit="mm",
    ),
    OptimizerVariable(
        "y_feed",
        initial_value=-2.655455277228904,
        lower_bound=-6,
        upper_bound=0,
        unit="mm",
    ),
...
CONSTRAINTS = [
    SParameterUpperBoundConstraint(
        s_parameter="S11",
        upper_bound_db=-40,
        lower_frequency_hz=constants.ghz(8.9),
        upper_frequency_hz=constants.ghz(9.1),
    ),
]
...
if __name__ == "__main__":
    ...
    flags.DEFINE_string("design_name", "patch_antenna_ro4350b_4L_sma",
                        "HFSS design name.")
