...
class PronyMpmComplexExponentialEstimatorTestCase(absltest.TestCase):
    ...
    def test_decaying_exponential(self):
        decaying_exponential = ComplexExponential(SAMPLING_FREQUENCY,
                                                  ...
                                                  alpha=np.log(0.5),
                                                  snr=np.inf)
        estimator = PronyMpmComplexExponentialEstimator(decaying_exponential,
                                                        SAMPLING_FREQUENCY)
    ...
    def test_sinusoid(self):
        sinusoid = ComplexExponential(SAMPLING_FREQUENCY,
                                      ...
                                      alpha=0,
                                      snr=np.inf)
        estimator = PronyMpmComplexExponentialEstimator(sinusoid,
                                                        SAMPLING_FREQUENCY)
        params = estimator.estimate_single_exponential()
        ...
        self.assertIsNone(np.testing.assert_allclose(params.amplitude, 2))
        self.assertIsNone(np.testing.assert_allclose(params.alpha, 0,
                                                     atol=1e-9))
    ...
    def test_multiple_decaying_exponentials(self):
        ...
        decaying_exponential2 = ComplexExponential(SAMPLING_FREQUENCY,
                                                   ...
                                                   alpha=np.log(0.8),
                                                   snr=np.inf)
        estimator = PronyMpmComplexExponentialEstimator(
...
class PronyMpmNoiseComplexExponentialEstimatorTestCase(absltest.TestCase):
    ...
    def test_decaying_exponential(self):
        decaying_exponential = ComplexExponential(SAMPLING_FREQUENCY,
                                                  ...
                                                  alpha=np.log(0.5),
                                                  snr=np.inf)
        estimator = PronyMpmNoiseComplexExponentialEstimator(
    ...
    def test_multiple_decaying_exponentials(self):
        ...
        self.assertIsNone(
            np.testing.assert_allclose(params[0].frequency, 0, atol=1e-9))
        self.assertIsNone(
            np.testing.assert_allclose(params[0].phase, 0, atol=1e-8))
        self.assertIsNone(np.testing.assert_allclose(params[0].amplitude, 3))
        self.assertIsNone(
            np.testing.assert_allclose(params[0].alpha, np.log(0.5)))
        self.assertIsNone(
